# Figures 8 and 9: v2 padded

The approved v2 copy and design, with larger internal padding, more line spacing, and taller boxes. Existing v2s and article references are unchanged.

Comparisons use equal widths: v2 above padded on desktop, v2 left of padded on mobile.

## Figure 8

### Desktop

[Padded SVG](../../public/media/then-what-v2-padded.svg)

![Figure 8: v2 and v2 padded, desktop](figure-8-desktop-comparison.png)

### Mobile

[Padded SVG](../../public/media/then-what-v2-padded.mobile.svg)

![Figure 8: v2 and v2 padded, mobile](figure-8-mobile-comparison.png)

## Figure 9

### Desktop

[Padded SVG](../../public/media/failure-modes-v2-padded.svg)

![Figure 9: v2 and v2 padded, desktop](figure-9-desktop-comparison.png)

### Mobile

[Padded SVG](../../public/media/failure-modes-v2-padded.mobile.svg)

![Figure 9: v2 and v2 padded, mobile](figure-9-mobile-comparison.png)

Regenerate: `python3 -B scripts/figures_v2_padded.py`. Add `--render-review` to render the comparison PNGs with Inkscape.
