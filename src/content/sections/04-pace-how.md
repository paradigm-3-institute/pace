...
![The tradeoff between speed, legitimacy and precision when triggering pacing interventions.](/media/pacing-tradeoffs-venn.svg)
...
**Ending pacing** here is uncertain. If the cap is buying time usefully, then exiting may be desired once humanity has sufficient assurance that it can avoid loss of control, or that the benefits of development exceed the risks. Progress by non participants may also change the calculus regarding whether persisting with restrictions is beneficial. Staged exits could be used to minimize the risks of sudden jumps caused by a capabilities overhang.
